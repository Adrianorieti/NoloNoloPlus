...
export default {
  ...
  mounted(){
    ...
    let url = 'http://localhost:8001/api/customer/category';
  ...
  methods: {
    ...
    changeChart(){
      ...
      for (let cat of this.categories){
        let filteredProducts = this.products.filter(prod => prod.type === cat.name);
        labels.push(cat.name);
        if(this.chartView === 'numberOfRents' || this.chartView === 'totalSales' ){
          let sum = 0;
